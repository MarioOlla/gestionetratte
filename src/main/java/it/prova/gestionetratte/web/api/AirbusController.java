...
public class AirbusController {
	...
	public List<AirbusDTO> listAll(){
		System.out.println("sono arrivato dentro il controller");
		return AirbusDTO.buildAirbusDTOListFromModelList(airbusService.listAllEager(), false);
	}
	...
	public List<AirbusDTO> listAllConSovrapposizione(){
		...
		.forEach(airbus -> {
			if(this.hasSovrapposizioni(airbus))
				result.add(AirbusDTO.buildAirbusDTOFromModelConSovrapposizioni(airbus, false));
			else
				result.add(AirbusDTO.buildAirbusDTOFromModel(airbus, false));
		});		
	...
	//metodo di utilità che controlla se ci sono sovrapposizioni tra le tratte di un airbus:
	//per ogni tratta dell'airbus controlla se l'ora di decollo o l'ora di atterraggio cadono
	//tra l' ora di decollo e di atterraggio delle altre tratte; se viene verificata questa 
	//condizione la funzione ritorna true, altrimenti passa all' elemento successivo, se 
	//terminano gli elementi da analizzare senza che la condizione si sia verificata la funzione
	//restituisce false.
		...
		for (int i = 0; i < tratte.size(); i++) {
			...
			for (int j = i; j < tratte.size(); j++) {
				...
				if(i==j)
					...
					if(tratta1.getData().isEqual(tratta2.getData()))
