...
public class CustomTrattaRepositoryImpl implements CustomTrattaRepository{
	...
	public List<Tratta> findByExample(Tratta example) {
		...
		StringBuilder queryBuilder = new StringBuilder("select t from Tratta t left join fetch t.airbus a where t.id = t.id ");
		...
		if(example.getAirbus() != null && StringUtils.isNotBlank(example.getAirbus().getCodice())) {
			whereClauses.add(" a.codice like :codice");
			paramaterMap.put("codice", "%"+example.getAirbus().getCodice()+"%");
		}
